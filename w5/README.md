...
Scripts in `scripts/` provide operational entry points for deployment, state inspection, transfer submission, and extension management: `deploy.tolk`, `info.tolk`, `send-transfer.tolk`, and `manage-extensions.tolk`. Run them with `acton script w5/scripts/<name>.tolk`.
